...
The code for this project can be found on the following GitHub repository: <a href="https://github.com/wrains/ToolsProjectMEM">"https://github.com/wrains/ToolsProjectMEM"</a>
...
```
Here we can see a fairly comparable result between branches, but the takeaway here should be that at both branches there are less women than men. At the London branch we see a higher % difference comparing men to women 

This could manifest in women in London feeling a bit more ostracized and the feeling of being in a man's world. as often is felt by women in many industries including IT.
...
### Outside data conclusion

This is quite concerning for the company as we see a > 10% pt difference between the top levels of what typical in industry for work life balance. Comparatively to the industry we'd think that among employees the company would not be seen favorably for work life balance. To see this gap especially for women is equally concerning as the gap is larger than that for men. 

This could lead to issues for the company to attain and retain top talent as clearly other companies rank better in this regard. 
...
### Concern 1 Conclusion

In many regards in looking into these items we see that there are very minimal differences and only a few key takeaways moving forward. The key takeaways being:

- In general the company could at both branches promote the hiring of more women to reach greater equality in the workplace. Maybe some outreach here at the collegiate level with women engineering organizations or even at the high school level would attract more women to company in the future. London actually has a further discrepancy in the regard despite the CEO seeing London as an example moreso - maybe she needs to view the situation less as Barcelona branch needing to reach to London's structure. 
- The findings from our research on outside data are particularly compelling here and should be of focus. Although the company does fall in line for the lowest work life balance, it is clear that at the top levels of work-life balance, the company does not compare well to the rest of the industry. This could cause major problems in hiring and retaining top talent as this is a very critical component that potential employees take into account. Particularly with websites such as Glassdoor.com we presume that it is clear to people that do some research that this is indeed the case.
...
env_sat_RnD <- myData%>%filter(department=="Research and Development" & attrition!="")
...
env_sat_RnD <- myData%>%filter(department=="Research and Development" & over_time!="")
...
env_sat_RnD <- myData%>%filter(department=="Research and Development" & attrition!="" & attrition == "Yes")
...
ggplot(percentages,
       aes(x=departments,y=perc_env_satis,fill=departments)) +
  geom_bar(position="dodge",stat = "identity")  +
  labs(x = "Attrition = Yes + Over time = Yes",
       fill = "Department",
       y="Percentage") +
  scale_fill_manual(values=c("#177879", "#fff689", "#165797")) +
  coord_flip()
...
env_sat_RnD <- myData%>%filter(department=="Research and Development")
...
ggplot(datas,
  ...
  labs(x = "Department",
       fill = "Department",
       y="Average percentage salary hike") +
  scale_fill_manual(values=c("#177879", "#fff689", "#165797")) +
  coord_flip()
```
...
ggplot(datas,
  ...
  labs(x = "Department",
       fill = "Department",
       y="Average years in current role") +
  scale_fill_manual(values=c("#177879", "#fff689", "#165797")) +
  coord_flip()
```
...
### Conclusion concern 2

In many regards in looking into these variables of data we see that there are very minimal differences between the different departments of the company. Therefore there should not be a concern as long as these variables are maintained under the same circumstances.
...
In order to determine which factors cause a person to leave the company, an attrition model will be performed.

### Attrition model

It is important which are the variables to take into account that make a person want to leave the company. For this reason, it was decided to create a linear regression model where we can determine the most influential variables for a person to make this decision.

The variable to determine in the model is "attrition" and in a first moment all the other variables will be taken.
...
Taking these variables into account, a random forest model will be used to validate that these variables really affect the attrition variable.
...
```
Let's check at the performance of the Random Forest on the Train Set
...
```
The Accuracy of the model is 0.89 which is pretty high, that means the model works fine.

Now that we have seen, the model works perfectly on the Train Set we have to validate it using the Test Set.
...
atr_ranger_wf %>%
  ...
  conf_mat(truth = attrition, estimate = .pred_class)
```
The metrics of the Test Set as below.
...
atr_ranger_wf %>%
  ...
  kbl(caption = "Random Forest Model Performance - Test") %>%
      kable_classic(full_width = F)
```
The accuracy of the Test Set is almost 10 points lower than that of the Train Set. This is 0.82, although it has decreased, it can be concluded that the model fits very well, considering that an accuracy of this level is good.

Therefore, it is concluded that the variables that have been chosen in the first moment are correct and directly affect the attrition of people.
...
```{r}
activeEmployees%>%tabyl(department,job_role,job_level)

depts=activeEmployees%>%filter(department!="")%>%pull(department)%>%unique()
for (dept in depts) {
  print(dept)
  print(activeEmployees %>% filter(department==dept)%>%tabyl(job_role, job_level))
  
}
```
In view of these results, we can determine which roles are the highest looking at which have the lowest percentage of people.

As you can see the top roles in the Research & Development department from highest to lowest are:
- Manager
- Research Director

After these we find Healthcare Representative and Manufacturing Director, which are at the same level.

In the case of the Sales department:
- Manager
...
We will now try to search candidates that could be promoted in to a higher role.

We are going to start with those who has a job level equal to 4 out of 5.

##### Job Level 4

```{r}
depts1 =activeEmployees%>%filter(department!="") %>% filter(job_level == 4)%>%unique()

#The plot
ggplot(data = depts1, aes(x = department)) +
  geom_bar(aes(fill = department)) + 
  labs(title = "Employees at Level 4 by Department",
       x = "Departments",
       y = "Employees at level 4") +
  scale_fill_manual(values=c("#e03616", "#fff689", "#149858"))
```

There is a lot of people in job level 4, so now we need to filter using more variables to find the one we want.
As explained before we will need to filter by job satisfaction, performance rating and years since last promotion. we establish that we need to find the perfect one, so the job satisfaction and the performance rating has to be 4.

```{r}
depts1 =activeEmployees%>%filter(department!="") %>% filter(job_level == 4) %>% filter(job_satisfaction == 4) %>% filter(performance_rating == 4)%>%unique()

ggplot(data = depts1, aes(x = department)) +
  geom_bar(aes(fill = department)) +
  labs(title = "Num. Candidates for a Promotion",
       x = "Departments",
       y = "Candidates for a Promotion") +
  scale_fill_manual(values=c("#e03616", "#fff689", "#149858"))

depts1 %>% tabyl(department)

```
Applying the filters mentioned above, we found 1 candidate for promotion in the Sales and Human Resources departments. For Research & Development we have 6 candidates. Although there are many more than in the other departments, there are also many more people in this one. However, we will filter by years since last promotion to see how many candidates should be promoted first.

```{r}
depts1 =activeEmployees%>%filter(department!="") %>% filter(job_level == 4) %>% filter(job_satisfaction == 4) %>% filter(performance_rating == 4) %>% filter(department == "Research & Development")%>%unique()

ggplot(data = depts1, aes(x = years_since_last_promotion)) +
  geom_bar(aes(fill = years_since_last_promotion),fill="#00bfc2") +
  labs(title = "Years Since Last Promotion of the Candidates",
       x = "Years Since Last Promotion",
       y = "Candidates for a Promotion") + scale_x_continuous(breaks = c(0:11))+
  guides(fill="none") 

depts1 %>% tabyl(years_since_last_promotion)
```
In the case of this department, the person to be promoted is the one who has not been promoted for 11 years. 

Now let's look at job level 3.

##### Job Level 3

The philosophy for choosing candidates will be the same as in the previous case. This time we will go straight to the point and look at which profiles can be promoted, based on job satisfaction, performance rating and if necessary we will look at the years since the last promotion in each department, as we have done in the previous case.

```{r}
depts2 =activeEmployees%>%filter(department!="") %>% filter(job_level == 3) %>% filter(job_satisfaction == 4) %>% filter(performance_rating == 4)%>%unique()

ggplot(data = depts2, aes(x = department)) +
  geom_bar(aes(fill = department)) +
  labs(title = "Num. Candidates for a Promotion",
       x = "Departments",
       y = "Candidates for a Promotion") +
...
depts2 %>% tabyl(department)
```
In this case we only have candidates in two departments, in the Sales department and in the Research & Development department, with 2 and 4 candidates respectively.

Let's take a look at the ones in Research & Development first.

```{r}
depts2 =activeEmployees%>%filter(department!="") %>% filter(job_level == 3) %>% filter(job_satisfaction == 4) %>% filter(performance_rating == 4) %>% filter(department == "Research & Development")%>%unique()
ggplot(data = depts2, aes(x = years_since_last_promotion)) +
  geom_bar(aes(fill = years_since_last_promotion), width = 0.75,fill="#00bfc2") +
  labs(title = "Years Since Last Promotion of the Candidates (R&D)",
       x = "Years Since Last Promotion",
       y = "Candidates for a Promotion") +theme(axis.text.x = element_text(vjust = 0.5, hjust=1))+scale_x_continuous(breaks = c(0:12))+
  guides(fill="none")

depts2 %>% tabyl(years_since_last_promotion)
```
In this case, the person who has been 11 years since the last promotion should be the first to obtain this promotion.

It's turn to take a look at the Sales department and the candidates in job level 3.
```{r}
depts2 =activeEmployees%>%filter(department!="") %>% filter(job_level == 3) %>% filter(job_satisfaction == 4) %>% filter(performance_rating == 4) %>% filter(department == "Sales")%>%unique()
ggplot(data = depts2, aes(x = years_since_last_promotion)) +
  geom_bar(aes(fill = years_since_last_promotion), width = 0.75,fill="#00bfc2") +
  labs(title = "Years Since Last Promotion of the Candidates (Sales)",
       x = "Years Since Last Promotion",
       y = "Candidates for a Promotion") +theme(axis.text.x = element_text(vjust = 0.5, hjust=1))+scale_x_continuous(breaks = c(0:12))

depts2 %>% tabyl(years_since_last_promotion)
```
We only have two candidates, one of them has been 0 years since his promotion and the other has been 2, therefore the second one should likely get the next promotion. 

<br>

If more experience is sought then the company should consider an outside hire in this case. 

Also with the above analysis the company should gauge if those people meet what they want in the higher position. If not or if the interest is not there on the side of the employee then the company should consider an outside hire. 

### Conclusions Concern 3

As shown at the beginning of this section, the Research & Development department is the most affected in terms of attrition. The model that has been generated is important because it will help to anticipate events when a person decides to leave the company.
This should also be taken into account in our succession plan. Where effort and years of experience within the company are rewarded. 

# Conclusions

At the beginning of the project we were given three different tasks, which have been developed throughout the project (Concern 1, Concern 2 and Concern 3). The objective was to analyze the current Human Resources status of the company. To carry out this objective, the new CEO of the company gave us a dataset with all the employees.
After cleaning this dataset and understanding all the variables that were in it, we proceeded to develop the three concerns.
The CEO of this company said that she believed very much in gender equality, and that the Barcelona office had to look like the London office because there were discrepancies. After a thorough study it has been found that this is totally the opposite, and that in the Barcelona office there is more gender equality than in London.
Subsequently we have studied the attitudes of the staff in all the different departments and the differences between departments is very minimal, so it should not be a cause for concern as they are all maintained in the same way. 
Finally, a succession plan has been drawn up to determine which people in the company are suitable for promotion, taking into account several variables but above all rewarding effort, performance and perseverance.
...
[4] Firke, S. (2021, January 5). CRAN - Package janitor. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/janitor/index.html

[5] Comprehensive R Archive Network (CRAN). (2021, October 7). CRAN - Package lubridate. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/lubridate/index.html

[6] Introduction to broom. (2021, October 31). Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/broom/vignettes/broom.html

[7] Arnold, J. B. (2021, January 20). CRAN - Package ggthemes. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/ggthemes/index.html

[8] Comprehensive R Archive Network (CRAN). (2021a, October 1). CRAN - Package tidymodels. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/tidymodels/index.html

[9] Rudis, B. N. R. A. S. G. (2021, October 13). Introduction to the viridis color maps. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html

[10] Rudis, B. (2020, March 6). CRAN - Package hrbrthemes. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/hrbrthemes/index.html

[11] Wickham, H. (2021, January 27). CRAN - Package forcats. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/forcats/index.html

[12] Comprehensive R Archive Network (CRAN). (n.d.). CRAN - Package caTools. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/caTools/index.html

[13] Zhu, H. (2021, February 19). Create Awesome HTML Table with knitr::kable and kableExtra. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_html.html

[14] Comprehensive R Archive Network (CRAN). (2021c, November 22). CRAN - Package yardstick. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/yardstick/index.html

[15] Vaughan, D. (2021, October 12). CRAN - Package workflows. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/workflows/index.html

[16] Comprehensive R Archive Network (CRAN). (2021a, July 21). CRAN - Package parsnip. Cran.r-Project.Org. Retrieved December 20, 2021, from https://cran.r-project.org/web/packages/parsnip/index.html
