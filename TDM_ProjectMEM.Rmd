...
And the file with the Business case was downloaded from the platform Atenea, in the Tools for Decision Making - 2021/2022 course for Master's Degree in Technology and Engineering Management offered by the Universidad Politecnica de Cataluña.
...
maleBCN <- myData %>% filter(city == "Barcelona" & gender == "Male") %>% pull(city) %>% length()
maleLDN <- myData %>% filter(city == "London" & gender == "Male") %>% pull(city) %>% length()

myData %>% tabyl(gender)
cityTotals <- c(totalBCN,totalBCN,totalLDN,totalLDN)

myData %>% group_by(city,gender) %>% count(gender) %>% cbind(cityTotals=cityTotals) %>% mutate(pct = n/cityTotals) %>% 
...
p4 <- myData %>% group_by(gender,city) %>% count(gender) %>%
...
Linear model  for mothly income : 
...
ABC_analysis_gender_A <- mutate(ABC_analysis1,Gender)
