...
myData %>% ggplot(aes(x=gender)) + geom_bar()
...
myData$department= ifelse(myData$department == "Reseach and Development", "Research & development", as.character(myData$department))
...
```{r}
ggplot(myData, aes(job_role)) + geom_bar()  + coord_flip()
```
...
As for the gender analysis with ABC method : 
...
Now we will do the analysis for London city: 
...
bcnActive%>%filter(department!="")%>%group_by(department)%>%select_if(is.numeric)%>%summarise_all(mean)
ldnActive%>%filter(department!="")%>%group_by(department)%>%select_if(is.numeric)%>%summarise_all(mean)
...
```{r}
#Roger's plots
ggplot(myData, aes(x=num_companies_worked, y=monthly_income, color= department)) + geom_point() + geom_smooth(method=lm)
ggplot(myData, aes(x=over18))+geom_bar()
ggplot(myData, aes(x=num_companies_worked, y=percent_salary_hike)) + geom_point() + geom_smooth(method=lm)
```
...
ggplot(people_left_bcn, aes(x=department)) + geom_bar(stat="count", fill=alpha("navyblue", 0.7))  +
...
ggplot(people_left_ldn, aes(x=department)) + geom_bar(stat="count", fill=alpha("navyblue", 0.7))  +
  theme_minimal() + coord_polar(start = 0)
